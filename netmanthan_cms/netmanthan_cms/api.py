...
def get_template_folder(url=None, business=None, temp=0):
	try:
		if not url:
			path = frappe.get_module_path("netmanthan_cms")
...
def get_section_data(section, page_builder, device_type):
	from netmanthan_cms.netmanthan_cms.doctype.web_page_builder.web_page_builder import  get_source_doc_data, get_page_section
...
def check_domain(domain_name):
	try:
		...
		frappe.log_error(frappe.get_traceback(), 'netmanthan_cms.netmanthan_cms.api.check_domain')
...
def update_website_context(context):
	try:
		...
		frappe.log_error(frappe.get_traceback(),"netmanthan_cms.netmanthan_cms.api.update_website_context")

def get_device_type(context):
	try:
		from netmanthan_cms.netmanthan_cms.device_detect.detect import detect_mobile_browser
		from netmanthan_cms.netmanthan_cms.device_detect.utilities import get_user_agent
		ua=None
		try:
		...
		context.device_type="Desktop"
		frappe.log_error(frappe.get_traceback(), "netmanthan_cms.netmanthan_cms.api.get_device_type") 	
