...
frappe.ui.form.on('Web Theme', {
	...
	pick_color_palette(frm){
		frappe.call({
			method:"netmanthan_cms.netmanthan_cms.doctype.web_theme.web_theme.get_color_palette_datas",
			args:{},
			freeze:true,
			callback:(res) =>{
	...
	update_default_values(frm,res){
		frappe.call({
			method:"netmanthan_cms.netmanthan_cms.doctype.web_theme.web_theme.get_color_palette_default_datas",
...
frappe.ui.form.on('Typography Heading', {
	...
	edit_styles(frm,cdt,cdn){
		...
		if(item.type){
			frappe.call({
				method:"netmanthan_cms.netmanthan_cms.doctype.web_theme.web_theme.get_json_render_properties",
				...
				if(each_list.child_properties && each_list.child_properties.length > 0){
					each_list.child_properties.map(ls =>{
						if(ls.hasOwnProperty('variable_name')){
							if(ls.variable_name == "--netmanthan-button-$-gc"){
